...
describe("Todo Application", function () {
  ...
  test("Deletes a todo with the given ID if it exists and sends a boolean response", async () => {
    ...
    // Now, delete that todo by ID
    const deleteResponse = await agent.delete(`/todos/${todoID}`).send();
  
    // Check that response is true (success)
    expect(deleteResponse.statusCode).toBe(200);
    expect(deleteResponse.text).toBe("true");
  
    // Try to delete the same todo again - should return false (already deleted)
    const deleteAgainResponse = await agent.delete(`/todos/${todoID}`).send();
    expect(deleteAgainResponse.statusCode).toBe(200);
    expect(deleteAgainResponse.text).toBe("false");
  });
